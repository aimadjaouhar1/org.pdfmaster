...
export class PdfService {
    ...
    async extract(pageIndices: number[], file: Express.Multer.File, @Res() response: Response) {
        const buffer = fs.readFileSync(`${file.destination}/${file.filename}`);
        const pdfDoc = await PDFDocument.load(buffer);
        const filename = file.originalname.replace('.pdf', '');

        const newDoc = await PDFDocument.create();
        (await newDoc.copyPages(pdfDoc, pageIndices)).forEach(page => newDoc.addPage(page));

        const newFile = await newDoc.save();

        response.contentType('application/pdf');
        response.attachment(`${filename}_extracted.pdf`);
        response.send(Buffer.from(newFile));
    }
